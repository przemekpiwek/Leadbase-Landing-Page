(function ($, undefined) {
  ...
  $.fn.ui_app_showcase = function () {
    ...
    win.on("resize", function () {
      if ($(this).width() >= breaks.sm) {
        ...
        showcase.html(
          col_img +
            '<div class="col-xs-8" data-vertical_center="true"><div class="row">' +
            col_txt_a +
            col_txt_b +
            "</div></div>"
        );
  ...
  $.fn.ui_device_slider = function () {
    ...
    $('[data-toggle_slider="' + id + '"][data-toggle_screen]').on(
      "click",
      function () {
        y = true;
        if (!z) {
  ...
  $.fn.ui_video_player = function () {
    var video_modal = $(
      '<div class="video-modal modal" role="dialog"><div class="dialog container"><a class="close" data-close="modal" aria-hidden="true">×</a><div class="video-player"></div></div></div>'
    );
    var toggle = this;
    toggle.on("click", function () {
      ...
      $(".video-modal")
        .find(".video-player")
        .html(
          '<div class="video-player"><iframe src="https://www.youtube-nocookie.com/embed/' +
            youtube_id +
            '?rel=0&amp;showinfo=0&amp;autoplay=1" frameborder="0" allowfullscreen></iframe></div>'
        );
  ...
  $.fn.ui_instagram_feed = function () {
    ...
    if (auth_token === "YOUR_AUTHTOKEN" || auth_token.length === 0) {
      console.error(
        'Instgram Widget: - You need to place your auth token in the widget\'s "data-authtoken" attribute.'
      );
      valid = false;
    }
    if (user_id === "YOUR_USERID" || user_id.length === 0) {
      console.error(
        'Instgram Widget: - You need to place your user id in the widget\'s "data-userid" attribute .'
      );
      valid = false;
    }
    if (valid) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status === "success") {
          ...
          var msg = $(
            '<span class="help-block form-success" style="margin-bottom: 0;margin-top: 1rem"/>'
          ).text(data.message);
          if ($("#contact-form").length > 0) {
            msg = $(
              '<span class="help-block form-success" style="margin-bottom: 1rem;margin-top: 1rem"/>'
            ).text(data.message);
  ...
  win.on("resize", function () {
    if (win.width() >= 740) {
      $(".pricing-sidebar .pricing-header").height(
        $(".pricing-block .pricing-header").outerHeight()
      );
    } else {
